...
AUTH0_DOMAIN = 'test-abcd.us.auth0.com'
ALGORITHMS = ['RS256']
API_AUDIENCE = 'drinks'
...
def get_token_auth_header():
    ...
    if auth_header is None:
        raise AuthError({
            ...
            'description': 'No authorization header in the request'
        }, 401)
...
def check_permissions(permission, payload):
    ...
    if 'permissions' not in payload:
        raise AuthError({
            'code': 'bad_request',
            'descriptions': 'No Permissions on the payload'
        }, 400)

    # Check if this specific permission is present
    if permission not in payload['permissions']:
        raise AuthError({
            'code': 'not_authorized',
            'description': 'Permission not found in the payload'
        }, 401)
...
def verify_decode_jwt(token):
    # This code is boilerplate copied from Auth0 documentaion
